...
ControllerState state = {.state = SYNC_CONTROLLER, .report_count = 0, .echoes = 0};
ControllerState state2 = {.state = SYNC_CONTROLLER, .report_count = 0, .echoes = 0};
#define ECHOES 2
int echoes = 0;
...
void HID_Task(void) {
	...
	if (Endpoint_IsINReady()) {
		...
		GetNextReport(&JoystickInputData, &state2);
    ...
    // We'll start with the OUT endpoint.
    Endpoint_SelectEndpoint(JOYSTICK2_OUT_EPADDR);
    // We'll check to see if we received something on the OUT endpoint.
    if (Endpoint_IsOUTReceived()) {
        // If we did, and the packet has data, we'll react to it.
        if (Endpoint_IsReadWriteAllowed()) {
    ...
    // We'll then move on to the IN endpoint.
    Endpoint_SelectEndpoint(JOYSTICK2_IN_EPADDR);
    // We first check to see if the host is ready to accept data.
    if (Endpoint_IsINReady()) {
        // We'll create an empty report.
        USB_JoystickReport_Input_t JoystickInputData;
        // We'll then populate this report with what we want to send to the host.
        GetNextReport(&JoystickInputData, &state);
        // Once populated, we can output this data to the host. We do this by first writing the data to the control stream.
        while(Endpoint_Write_Stream_LE(&JoystickInputData, sizeof(JoystickInputData), NULL) != ENDPOINT_RWSTREAM_NoError);
        // We then send an IN packet on this endpoint.
        Endpoint_ClearIN();
...
void GetNextReport(USB_JoystickReport_Input_t* const reportData, ControllerState *cstate) {
    uint8_t data, i;
    ...
    switch (cstate->state) {
        case SYNC_CONTROLLER:
            if (cstate->report_count > 100) {
                cstate->report_count = 0;
                cstate->state = TATACON_PASSTHROUGH;
            }
            else if (cstate->report_count == 25 || cstate->report_count == 50) {
                reportData->Button |= SWITCH_L | SWITCH_R;
            }
            else if (cstate->report_count == 75 || cstate->report_count == 100) {
...
void EVENT_USB_Device_ConfigurationChanged(void) {
    ...
    ConfigSuccess &= Endpoint_ConfigureEndpoint(JOYSTICK2_OUT_EPADDR, EP_TYPE_INTERRUPT, JOYSTICK_EPSIZE, 1);
    ConfigSuccess &= Endpoint_ConfigureEndpoint(JOYSTICK2_IN_EPADDR, EP_TYPE_INTERRUPT, JOYSTICK_EPSIZE, 1);
